    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Selamat malam, {userName}. Aset Anda sedang dikelola secara otonom oleh AI.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: isFounder ? 'Total Aset Kelola' : 'Tabungan Sukarela', val: isFounder ? 'Rp 19.600M' : `Rp ${(user?.balances.voluntary || 15400000).toLocaleString('id-ID')}`, icon: '💰', color: 'text-indigo-600', bg: 'bg-white' },
          { label: isFounder ? 'Anggota Aktif' : 'Investasi Modal', val: isFounder ? '1,248' : `Rp ${((user?.balances.principal || 100000) + (user?.balances.mandatory || 1200000)).toLocaleString('id-ID')}`, icon: '👥', color: 'text-slate-800', bg: 'bg-white' },
        ...
        <div className="space-y-6">
           <div className="bg-slate-900 p-8 rounded-[3rem] text-white space-y-6 relative overflow-hidden shadow-2xl">
              ...
              <div className="space-y-3 relative z-10">
                 <button 
                  onClick={() => navigate(AppView.LOAN_SIMULATOR)}
                 ...
                 <button 
                  onClick={() => navigate(AppView.MEMBER_MARKETPLACE)}
                 ...
                 <button 
                  onClick={() => navigate(AppView.AI_ADVISOR)}
           ...
           <div className="p-8 bg-indigo-50 border border-indigo-100 rounded-[3rem] flex flex-col items-center text-center space-y-4">
              ...
              <button onClick={() => navigate(AppView.SMART_EDUCATION)} className="text-[10px] font-black text-indigo-600 underline uppercase">Mulai Belajar</button>
           </div>
        </div>
      </div>
    </div>
