  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500 pb-20 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col items-center">
         <div className="w-full max-w-md bg-white rounded-[4rem] shadow-2xl overflow-hidden border-[12px] border-slate-900 p-10 flex flex-col items-center space-y-10 relative">
            ...
            {!showToken ? (
               <div className="text-center space-y-10 py-10 animate-in zoom-in w-full">
                  <div className="w-40 h-40 bg-indigo-50 rounded-[3rem] flex items-center justify-center text-7xl shadow-inner mx-auto">🏪</div>
                  <div className="space-y-3 px-4">
                     <h3 className="text-2xl font-black text-slate-800">Siap Bayar?</h3>
                     <p className="text-slate-400 text-sm italic">"Pilih toko dan tunjukkan kode bayar ke kasir."</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                     <button onClick={generateToken} disabled={isGenerating} className="p-4 bg-indigo-50 rounded-2xl border-2 border-indigo-100 hover:border-indigo-600 transition-all flex flex-col items-center gap-2">
                        <span className="text-xs font-black text-indigo-600 uppercase tracking-widest">INDOMARET</span>
                     </button>
                     <button onClick={generateToken} disabled={isGenerating} className="p-4 bg-rose-50 rounded-2xl border-2 border-rose-100 hover:border-rose-600 transition-all flex flex-col items-center gap-2">
               ...
               <div className="w-full space-y-8 animate-in fade-in">
                  <div className="p-8 bg-slate-900 rounded-[3rem] flex flex-col items-center justify-center space-y-8 shadow-2xl relative overflow-hidden">
                     ...
                     <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] relative z-10">TOKEN PEMBAYARAN</p>
            ...
            {isGenerating && (
               <div className="absolute inset-0 bg-white/90 backdrop-blur-md z-30 flex flex-col items-center justify-center space-y-6 animate-in fade-in">
                  ...
                  <p className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em] animate-pulse">Handshake Network Retail...</p>
               </div>
            )}
         </div>
         ...
         <div className="mt-12 p-8 bg-indigo-50 border border-indigo-100 rounded-[3rem] text-center max-w-xl">
            <p className="text-xs text-indigo-900 leading-relaxed font-bold italic">
               "Setiap belanja di minimarket melalui KoperatifAI, Bapak/Ibu mendapatkan **Cashback SHU Rp 250** per transaksi. Rakyat mandiri, ekonomi bergulir."
            </p>
         </div>
      </div>
    </div>
