...
const MemberQRIS: React.FC = () => {
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      alert("Izin kamera diperlukan untuk belanja.");
  ...
  return (
    <div className="space-y-12 pb-24 max-w-4xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col items-center">
         <div className="w-full max-w-md bg-white rounded-[4rem] shadow-2xl overflow-hidden border-8 border-indigo-600 p-10 flex flex-col items-center space-y-10 relative">
            {!showScanner && !isSuccess ? (
               <div className="text-center space-y-10 animate-in zoom-in w-full">
               ...
               <div className="w-full space-y-8 animate-in fade-in">
                  <div className="aspect-square w-full bg-black rounded-[3rem] relative flex items-center justify-center border-4 border-slate-800 overflow-hidden">
                     <video ref={videoRef} autoPlay playsInline className="absolute inset-0 w-full h-full object-cover" />
                     <div className="absolute inset-0 border-[40px] border-black/40 pointer-events-none z-10"></div>
                     <div className="w-48 h-48 border-4 border-indigo-500 rounded-[2rem] relative z-20">
                  ...
                  <div className="p-4 bg-indigo-50 rounded-2xl text-center">
                     <p className="text-[10px] font-black text-indigo-600 uppercase">Mencari Kode QR...</p>
                     <p className="text-xs italic text-indigo-900">Bidik kotak sampai muncul nama warung.</p>
                  </div>
                  <button 
                    onClick={simulatePayment}
                    className="w-full py-6 bg-emerald-600 text-white rounded-2xl font-black uppercase text-xs shadow-lg"
                  >
                     SIMULASI QR TERDETEKSI
                  </button>
